...
def _prompt_review_pending_change(vault: str, change_id: str) -> dict:
    text = (
        ...
        "   - NOTE: `validation_status` and `validation_errors` reflect the state\n"
        "     persisted when the proposal was created. This view does not re-run\n"
        "     schema validation. Acceptance re-validates before any vault write,\n"
        "     so an invalid proposal cannot reach a vault note.\n\n"
